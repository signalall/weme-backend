...
public interface PersonImageDao extends CrudRepository<PersonalImage, Long> {
    ...
    @Query(value = "select from PersonalImage as personImage where personImage.user.id = :userId and " +
    ...
    @Query(value = "select from PersonalImage as personImage where personImage.user.id = :userId and personImage.disable = false")
